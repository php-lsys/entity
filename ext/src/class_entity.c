...
static int get_db_builder(zval *object,zval *db_builder){
    zend_call_method_with_0_params(object,Z_OBJCE_P(object), NULL, "builder", db_builder);
...
ZEND_METHOD(lsentity_entity_class, update){
    ...
    if(!get_db_builder(&db,&dbbuilder)){
        zval_ptr_dtor(&csave_data);
        zval_ptr_dtor(&table);
        zval_ptr_dtor(&db);
        RETURN_NULL();
            ...
            if(lsentity_obj_check(lsentity_column_ce_ptr,val,0,0)){
                ...
                if(!lsentity_check_bool_with_0_params(val,"isallownull")
                   ...
                   &&ZVAL_IS_NULL(dfind)){
                    ...
                    Z_REFCOUNTED(tmp)&&Z_ADDREF(tmp);
                    zend_hash_update(Z_ARR_P(data),Z_STR(name),&tmp);
                    zval_ptr_dtor(&tmp);
                }
                create_column_end:
                zval_ptr_dtor(&name);
